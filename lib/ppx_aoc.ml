...
let match_impl (ld : label_declaration) =
...
let match_intf ~ptype_name (ld : label_declaration) =
  let loc = ld.pld_loc in
  psig_value
    ~loc
...
let generate_impl ~ctxt (_rec_flag, type_declarations) =
  ...
  List.map type_declarations ~f:(fun (td : type_declaration) ->
    ...
    | { ptype_kind = Ptype_record fields; _ } -> List.map fields ~f:match_impl)
  |> List.concat
;;
...
let generate_intf ~ctxt (_rec_flag, type_declarations) =
  ...
  List.map type_declarations ~f:(fun (td : type_declaration) ->
    ...
    | { ptype_kind = Ptype_record fields; ptype_name; _ } ->
      List.map fields ~f:(match_intf ~ptype_name))
  |> List.concat
;;
...
let my_deriver =
  Deriving.add "aocmatch" ~str_type_decl:impl_generator ~sig_type_decl:intf_generator
;;
